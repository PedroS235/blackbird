...
1. FFT the setpoint (`S`) and the filtered gyro (`G`), zero-padded to avoid
...
| Window | `2 * Fs` samples (2 s) | Long enough for low frequencies, short enough that the tune is constant across it |
| Hop | `Fs / 4` (0.25 s, 87% overlap) | Dense stack for the all-traces view; ~80 traces per axis on a 20 s log |
| λ | `k · mean(|S|²)`, `k = 0.01` | Scale-relative, so it behaves the same on a cruise and on a flip |
| Window rejection | `max |setpoint| < 20 deg/s` | No stick input means the deconvolution is noise |
