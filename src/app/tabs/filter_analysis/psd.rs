...
use crate::app::tabs::{get_axis_color, stacked_plot_height_of};
...
impl Psd {
    pub(super) fn show(&mut self, ui: &mut Ui, analysis: &SpectralAnalysis) {
        // Captured before the scroll area, whose content height a `Ui`
        // reports as unbounded — this keeps the plots the size they've
        // always been in the common case, with the scroll area itself only
        // kicking in as the safety net for whatever that estimate misses.
        let available = ui.available_height();
        let plot_height =
            stacked_plot_height_of(available, drawn_axes(analysis), ai_feedback::RESERVE_HEIGHT);
